...
function _createForOfIteratorHelper(o, allowArrayLike) {
  var it; // 이터레이터 객체를 저장할 변수

  // 반복 가능한 객체를 처리하고 이터레이터를 생성하는 부분
  if (typeof Symbol === "undefined" || o[Symbol.iterator] == null) {
     if (Array.isArray(o) || (it = _unsupportedIterableToArray(o)) || allowArrayLike && o && typeof o.length === "number") {
        if (it){
           o = it; // 이터레이터로 변환
        } 
        var i = 0; // 현재 인덱스를 나타내는 변수
        var F = function F() {}; // 빈 함수 객체

        // 반환하는 객체
        return {
           s: F, // 이터레이터 초기화 함수
           n: function n() { // 다음 요소를 반환하는 함수
              if (i >= o.length) {
                 return { done: true }; // 모든 요소를 순회한 경우
              }
              return { done: false, value: o[i++] }; // 다음 요소 반환
           }, 
           e: function e(_e) { throw _e; }, // 예외를 던지는 함수
           f: F // 마지막에 호출되는 정리 함수
        };
     } 

     // 반복 가능한 객체가 아닌 경우 오류 처리
     throw new TypeError("Invalid attempt to iterate non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.");
  } 

  var normalCompletion = true, // 루프 정상 완료 여부
      didErr = false, // 예외 발생 여부
      err; // 예외 정보 저장 변수

  // 반환하는 객체
  return {
     s: function s() { it = o[Symbol.iterator](); }, // 이터레이터 초기화 함수
     n: function n() { // 다음 요소를 반환하는 함수
        var step = it.next();
        normalCompletion = step.done;
        return step;
     }, 
     e: function e(_e2) { // 예외를 던지는 함수
        didErr = true;
        err = _e2;
     }, 
     f: function f() { // 마지막에 호출되는 정리 함수
        try { 
           if (!normalCompletion && it["return"] != null) {
              it["return"](); 
           }
